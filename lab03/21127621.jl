...
function normalize(x, mean_=nothing, std_=nothing)
   if mean_ == nothing && std_ == nothing
    ...
    return (x .- mean_) ./ std_ #return `normalized_train_x` calculated by using mean_ and std_ (both of them are passed and not null)
end
...
function compute_gradient(error, train_x)
    ...
    """
    return return (train_x' * error) / size(train_x, 1)
end
